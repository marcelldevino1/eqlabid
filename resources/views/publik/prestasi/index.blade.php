<section id="prestasi"
  ...
  <!-- Fade atas/bawah -->
  <div class="absolute top-0 left-0 w-full h-16 pointer-events-none bg-gradient-to-b from-white/90 to-transparent">
  </div>
  <div class="absolute bottom-0 left-0 w-full h-16 pointer-events-none bg-gradient-to-t from-white/90 to-transparent">
  </div>
  ...
  <div class="relative max-w-6xl mx-auto" data-anim="rise" data-delay="220">
    <!-- edge mask -->
    <div
      class="absolute inset-y-0 left-0 w-12 pointer-events-none bg-gradient-to-r from-white to-transparent rounded-l-2xl">
    </div>
    <div
      class="absolute inset-y-0 right-0 w-12 pointer-events-none bg-gradient-to-l from-white to-transparent rounded-r-2xl">
    </div>

    <!-- SLIDER -->
    <div id="slider-prestasi" class="flex gap-6 pb-2 overflow-x-auto snap-x snap-mandatory scroll-smooth"
      style="scrollbar-width:none;-ms-overflow-style:none;">
      <style>
        #slider-prestasi::-webkit-scrollbar {
          display: none
        }
      </style>

      @foreach(($prestasis ?? collect())->take(5) as $prestasi)
        <a href="{{ route('publik.prestasi.show', $prestasi->id) }}"
          class="snap-start shrink-0 w-[86%] sm:w-[60%] md:w-[46%] lg:w-[33.333%] focus:outline-none" data-anim="pop"
          data-delay="{{ 260 + ($loop->index * 60) }}">
          <article
            ...
            <div class="relative bg-gray-100/60 aspect-[4/3]">
              @if($prestasi->foto)
                <img src="{{ $prestasi->foto_url }}" alt="Prestasi {{ $prestasi->prestasi }}" loading="lazy"
                  decoding="async" class="object-cover object-center w-full h-full">
              @else
                <img src="{{ asset('images/default-placeholder.jpg') }}" alt="No image" loading="lazy" decoding="async"
                  class="object-cover object-center w-full h-full">
              @endif

              <span
                class="absolute top-3 left-3 grid h-7 w-7 place-items-center rounded-full bg-black/60 text-[11px] font-bold text-white">
                {{ $loop->iteration }}
              </span>
              <span
                class="absolute px-2 py-1 text-xs text-yellow-700 bg-white rounded-full shadow top-3 right-3">🏆</span>
            </div>
            ...
            <div class="p-4 text-left">
              ...
              <div class="mb-2">
                <span
                  class="inline-block px-3 py-1 text-[11px] sm:text-xs font-medium text-blue-800 bg-blue-100 rounded-md">
                  {{ $prestasi->kelas }}
                </span>
              </div>
      ...
      <a href="{{ route('publik.prestasi.index') }}"
        class="snap-start shrink-0 w-[86%] sm:w-[60%] md:w-[46%] lg:w-[33.333%] focus:outline-none" data-anim="pop"
        data-delay="{{ 260 + (min(($prestasis ?? collect())->count(), 5) * 60) }}">
        <article

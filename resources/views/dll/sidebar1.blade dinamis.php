...
<nav class="flex flex-col flex-1 w-full gap-3 overflow-y-auto">
    @foreach($menuItems as $moduleSlug => $actions)
        <button onclick="toggleSubmenu('{{ $moduleSlug }}')" 
            ...
            <div class="flex items-center">
                ...
                <span class="font-medium nav-text">{{ ucfirst($moduleSlug) }}</span>
            </div>
        ...
        {{-- Submenu method_actions --}}
        <div id="submenu-{{ $moduleSlug }}" class="hidden mt-2 submenu ml-9">
            @foreach($actions as $action)
                <a href="{{ route($moduleSlug.'.'.$action) }}"
                   class="flex items-center w-full px-3 py-2 text-sm text-gray-600 transition-all rounded-lg submenu-btn hover:bg-gray-100">
                    ...
                    <span class="nav-text">{{ ucfirst($action) }}</span>
                </a>
            @endforeach
        </div>
    @endforeach

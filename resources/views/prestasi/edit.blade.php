...
<div class="max-w-2xl p-6 mx-auto bg-white rounded-lg shadow-md">
    ...
    <form action="{{ route('prestasi.update', $prestasi->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="nama" id="nama" value="{{ old('nama', $prestasi->nama) }}"
        ...
        <div class="mb-4">
            ...
            <input type="text" name="kelas" id="kelas" value="{{ old('kelas', $prestasi->kelas) }}"
                   class="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500" required>
        </div>
        
        <div class="mb-4">
            <label for="prestasi" class="block mb-1 text-sm font-medium text-gray-700">Prestasi (Judul Singkat)</label>
            <input type="text" name="prestasi" id="prestasi" value="{{ old('prestasi', $prestasi->prestasi) }}"
                   class="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea name="deskripsi" id="deskripsi" rows="3"
                      class="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500" required>{{ old('deskripsi', $prestasi->deskripsi) }}</textarea>
            <p class="mt-1 text-xs text-gray-500">Isi deskripsi lengkap prestasi yang dicapai.</p>
        </div>

        <div class="mb-4">
            <label for="foto" class="block mb-1 text-sm font-medium text-gray-700">Ganti Foto (Kosongkan jika tidak ingin ganti)</label>
        ...
        <div class="mb-4">
            ...
            <img src="{{ asset('storage/' . $prestasi->foto) }}" alt="Foto {{ $prestasi->nama }}" class="object-cover w-24 h-24 rounded shadow">
        </div>
        @endif
